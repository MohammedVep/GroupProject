...
class DBManager
{
    ...
    func insert( groupNames :String)
    {
        let persons = read()
        for p in persons
        {
            if p.groupNames == groupNames
            {
                return
            }
        }
    ...
    func read() -> [Group] {
        ...
        if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                let groupNames = String(describing: String(cString: sqlite3_column_text(queryStatement, 0)))
                emps.append(Group(groupNames: [String]))
                print("Query Result:")
                print("\(groupNames)")
            }
        } else {
